...
private:
  ...
  void handleMessages() {
    ...
    while(true) {
		  ISteamNetworkingMessage *incoming_msg = nullptr;
			int num_msgs = network_interface_->ReceiveMessagesOnPollGroup(poll_group_, &incoming_msg, 1);
  ...
  void onNewClient(SteamNetConnectionStatusChangedCallback_t *info) {
    ...
    if(network_interface_->SetConnectionPollGroup(info->m_hConn, poll_group_) != k_EResultOK) {
      // something went wrong with the poll group 
      network_interface_->CloseConnection(info->m_hConn, 0, nullptr, false);
      std::cout << "WARNING: we failed to accept a connection because it could not be added to a poll group" << std::endl;
      return;
    }
