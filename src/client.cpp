...
public:
  ...
  void start() {
    // init connection lib
      ...
      std::cout << "ERROR: Failed to Intialize Game Networking Sockets because: " << error_msg << std::endl;
      exit(1);
    }
    ...
    server_address.ParseString("127.0.0.1:25565");
		...
		connection_= network_interface_->ConnectByIPAddress(server_address, 1, &opt );
  ...
  void processIncomingMessages() {
    ...
    while(true) {
		  ISteamNetworkingMessage *incoming_msg = nullptr;
			int num_msgs = network_interface_->ReceiveMessagesOnConnection(connection_, &incoming_msg, 1);
      ...
      if(num_msgs < 0) {
        std::cout << "WARNING: we failed to get a message from the poll group" << std::endl;
      }
  ...
  void sendString(const std::string& msg) {
			network_interface_->SendMessageToConnection(connection_, msg.c_str(), (uint32)msg.length(), k_nSteamNetworkingSend_Reliable, nullptr );
  }
